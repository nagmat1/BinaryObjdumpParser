...
#We increase the limit of the recursion functions.
...
def getMain(c_out):
	...
	for line in c_out.splitlines():
		...
		if gecir == 1 :
			...
			if len(line)==0 :
				bos_setir =1
				print("Setir bosh ekeni")
		if gecir == 1 and "ret" in line: 
			print(esasy)
			return esasy	
		if gecir == 1 and bos_setir ==1 :
			return esasy
...
def getFunc(addr):
	...
	gecir = 0
	bos_setir = 0
	gozleyan = addr.__str__() + ':'
	for line in c_out.splitlines():
		if gozleyan in line:
			gecir =1
		if gecir == 1 :
			esasy += line + '\n'
			if "ret" in line or "nop" in line or len(line)==0:
				return esasy
	...
	for cmd in progr.splitlines():
		...
		if 'jal' in opcode and 'jalr' not in opcode :
			...
			if PrC not in h and gozleg_funct in func_ady:
				...
				print(int(PrC,16),spisok)
			print(cmd.split('\t')[3:4])
			print(adrestap(cmd.split('\t')[3:4]))
